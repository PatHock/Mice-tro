...
//        assertEquals(controller.getDbNotes(note.getMeasureId(), note.getMeasureIndex()).get(0), note);
        ...
        controller.addNote(note.getType(), Defs.Pitch.A4, Defs.Key.D_MAJOR, Defs.TimeSignature.FOUR_FOUR, note.getMeasureIndex(), note.getMeasureId());

        // try to add note with invalid pitch based on key
        try {
            controller.addNote(note.getType(), Defs.Pitch.A0_FLAT, Defs.Key.D_MAJOR, Defs.TimeSignature.FOUR_FOUR, note.getMeasureIndex(), note.getMeasureId());
